class CortesEspeciaisComponent {
    constructor() {
        this.container = null;
        this.orders = [];
    ...
    async render(container) {
        ...
        this.container.innerHTML = `
            <div class="dashboard-loader">
                ...
                <p>Carregando pedidos para Cortes Especiais...</p>
            </div>
        `;

        try {
            await this.loadCortesEspeciaisOrders();
            this.renderCortesEspeciaisPage();
        } catch (error) {
            console.error("Erro ao carregar página de Cortes Especiais:", error);
            this.container.innerHTML = `
                <div class="error-page">
                    <i class="fas fa-exclamation-triangle"></i>
                    <h2>Erro ao carregar</h2>
                    <p>Ocorreu um erro ao carregar os pedidos para Cortes Especiais.</p>
                    <button class="btn btn-primary" id="retry-cortes-especiais-button">Tentar Novamente</button>
                </div>
            `;
            document.getElementById('retry-cortes-especiais-button').addEventListener('click', () => {
                this.render(container);
            });
        }
    }

    async loadCortesEspeciaisOrders() {
        try {
            ...
            console.error("Erro ao carregar pedidos de Cortes Especiais:", error);
            throw error;
        }
    }

    renderCortesEspeciaisPage() {
        let html = `
            <div class="page-header">
                <h1><i class="fas fa-ruler-combined"></i> Cortes Especiais</h1>
            </div>
            
            <div class="filters-bar">
                <div class="filter-group">
                    <label for="product-search">Produto/Serviço:</label>
                    <input type="text" id="product-search" class="search-input" placeholder="Digite o nome do produto...">
                </div>
            </div>

            <div class="cortes-especiais-container">
                <div id="cortes-especiais-orders-container">
    ...
    setupEventListeners() {
        // Filtros
        document.getElementById('product-search').addEventListener('input', () => this.applyFiltersAndRender());

        // Ações dos pedidos
        const ordersContainer = document.getElementById('cortes-especiais-orders-container');
    ...
    applyFiltersAndRender() {
        ...
        const container = document.getElementById('cortes-especiais-orders-container');
        container.innerHTML = this.renderOrdersList(this.orders);
    }

    renderOrdersList(ordersToRender) {
        if (!ordersToRender || ordersToRender.length === 0) {
            return `<div class="no-orders-message">Nenhum pedido no setor de Cortes Especiais.</div>`;
        }

        let ordersHtml = '<div class="cortes-especiais-orders-grid">';
            ...
            ordersHtml += `
                <div class="search-order-card ${situacao.class}" data-id="${order.id}">
                    ...
                    <div class="card-footer">
                        ...
                        <div class="cortes-especiais-order-actions">
...
window.CortesEspeciaisComponent = CortesEspeciaisComponent; 