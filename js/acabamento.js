class AcabamentoComponent {
    ...
    async render(container) {
        ...
        this.container.innerHTML = `
            <div class="dashboard-loader">
                ...
                <p>Carregando pedidos para acabamento...</p>
            </div>
        `;

        try {
            await this.loadAcabamentoOrders();
            this.renderAcabamentoPage();
        } catch (error) {
            console.error("Erro ao carregar página de acabamento:", error);
            this.container.innerHTML = `
                <div class="error-page">
                    <i class="fas fa-exclamation-triangle"></i>
                    <h2>Erro ao carregar</h2>
                    <p>Ocorreu um erro ao carregar os pedidos para acabamento.</p>
                    <button class="btn btn-primary" id="retry-acabamento-button">Tentar Novamente</button>
                </div>
            `;
            document.getElementById('retry-acabamento-button').addEventListener('click', () => {
                this.render(container);
            });
        }
    }

    async loadAcabamentoOrders() {
        try {
    ...
    renderAcabamentoPage() {
        let html = `
            <div class="page-header">
                <h1><i class="fas fa-cut"></i> Acabamento</h1>
            </div>
            ...
            <div class="acabamento-container">
                <div id="acabamento-orders-container">
    ...
    setupEventListeners() {
        ...
        // Ações dos pedidos
        const ordersContainer = document.getElementById('acabamento-orders-container');
    ...
    applyFiltersAndRender() {
        ...
        const container = document.getElementById('acabamento-orders-container');
        container.innerHTML = this.renderOrdersList(this.orders);
    }

    renderOrdersList(ordersToRender) {
        if (!ordersToRender || ordersToRender.length === 0) {
            return `<div class="no-orders-message">Nenhum pedido no setor de acabamento.</div>`;
        }

        let ordersHtml = '<div class="acabamento-orders-grid">';
            ...
            ordersHtml += `
                <div class="search-order-card ${situacao.class}" data-id="${order.id}">
                    ...
                    <div class="card-footer">
                        ...
                        <div class="acabamento-order-actions">
...
window.AcabamentoComponent = AcabamentoComponent; 